...
from posterior_models.base_model import test_epoch
...
def plot_posteriors_and_log_probs(
    ...
    posterior_log_probs = posterior_log_probs.cpu().numpy()
    reference_log_probs = reference_log_probs.cpu().numpy()
...
def complete_model_evaluation(train_dir, settings, dataset, model, metrics, save_samples=True):
    ...
    for obs in range(1, 10):
        ...
        # posterior_samples = posterior_samples.to('cuda:0')

        # 将 prior_dist 的所有参数转移到CUDA
        print(f"The device of posterior_samples from evaluate_sbibm is: {posterior_samples.device}")
        ...
        for m in metrics:
            if m == 'ksd':
                ...
                score = metrics_dict[m](posterior_samples.cpu(), reference_samples.cpu()).item()
            result[m] = score
...
if __name__ == "__main__":
    ...
    metrics = ['c2st', 'mmd', 'posterior_mean_error', 'posterior_variance_ratio', 'median_distance']
    ...
    complete_model_evaluation(args.train_dir, settings, dataset, model, args.metrics)

    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"The running time of the evaluation_sbibm code is : {elapsed_time} seconds")