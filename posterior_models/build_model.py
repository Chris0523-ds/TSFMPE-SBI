...
from posterior_models.score_matching import ScoreDiffusion


# TODO: where to put this to avoid cyclic imports?
def build_model_from_kwargs(filename=None, settings=None, **kwargs):
    """
    Returns the built model (from settings file or rebuild from file).
    Extracts the relevant arguments (normalizing flow or continuous flow) from setting.
    Parameters
    ----------
    filename
    settings
    ...
    if filename is not None:
        d = torch.load(filename, map_location="cuda")
        model_type = d["metadata"]["train_settings"]["model"]["type"] # 是一个字符串flow_matching
        # print("the option is number 1 !!!!")
        # print("the type name is: ", model_type)
    else:
        model_type = settings["train_settings"]["model"]["type"] # flow_matching
        # print("the option is number 2 !!!!")
        # print("the type name is: ",model_type)

    if not model_type.lower() in models_dict:
        raise ValueError("No valid posterior model specified.")

    model = models_dict[model_type.lower()] # model的变量类型是 <class 'type'>
    print("Selected model class:", model)
    ...
    #     # if model_type.lower() == "normalizing_flow":
    #     #     settings["train_settings"]["model"]["posterior_kwargs"].update(
    #     #         settings["train_settings"]["model"].get("nf_kwargs", {})
    #     #     )
    #
    #     if model_type.lower() in ["flow_matching", "score_matching"]:
    #         # 把 cf_kwargs 的内容合并到 posterior_kwargs
    #         settings["train_settings"]["model"]["posterior_kwargs"].update(
