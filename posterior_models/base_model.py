...
import utils.torchutils
import math
from utils.trainutils import EarlyStopping
...
class Base:
    ...
    def __init__(
        ...
        if model_filename is not None:
            ...
            self.network_to_device(device) # 转换设备
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        """
        if self.optimizer_kwargs is not None: # 这里去掉了utils
            self.optimizer = get_optimizer_from_kwargs(
                self.network.parameters(), **self.optimizer_kwargs
            )
        if self.scheduler_kwargs is not None: # 这里去掉了utils
...
def train_epoch(pm, dataloader):
    # LossInfo 是 trainutils 里面的类，其内部的 def print_info(self, batch_idx) 有输出信息
    pm.network.train()
    # pm 这里的类型是 <class 'posterior_models.flow_matching.FlowMatching'>，一个flow matching实例
    loss_info = utils.trainutils.LossInfo(
        pm.epoch,
    ...
    for batch_idx, data in enumerate(dataloader):
        ...
        # compute loss
        loss = pm.loss(data[0], *data[1:]) # loss函数请注意在 flow_matching 类里
    ...
    # 这里是把所有的 train epoch, loss, time dataloader, time network 都输出完了
    # print("loss_info.get_avg is shown as:",loss_info.get_avg())
    return loss_info.get_avg() # 这是一个值
