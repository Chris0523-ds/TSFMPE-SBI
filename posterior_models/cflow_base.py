...
class ContinuousFlowsBase(Base):
    ...
    def sample_t(self, batch_size):
        ...
        # 这里 time_prior_exponent = 4 时， 得到的新的分布是 p(t’) = 5*t'^4, t' follows [0,1]
    ...
    def log_prob_batch(self, theta, *context_data, hutchinson=False):
        ...
        *context_data: list[torch.Tensor]
        ...
        context_data = [ctx.to(model_device) if torch.is_tensor(ctx) else ctx 
                   for ctx in context_data]
        integration_range = self.integration_range.to(model_device)
        ...
        _, theta_and_div_0 = odeint(
            lambda t, theta_and_div_t: self.rhs_of_joint_ode(
                t.to(model_device), theta_and_div_t.to(model_device), *context_data, hutchinson=hutchinson
            ),
            theta_and_div_init,
            torch.flip(
